                ...
                <div class="flex-1">
                    <div class="bg-white rounded-2xl border border-purple-100 overflow-hidden">
                        <div class="px-5 py-4 border-b border-purple-50 flex items-center justify-between">
                            ...
                            <span class="text-sm text-gray-400">Choose the items to check out</span>
                        </div>

                        @foreach($cart->items as $item)
                            <div class="flex items-start gap-4 p-5 border-b border-purple-50 last:border-0 hover:bg-purple-50/30 transition">
                                <label class="pt-7">
                                    <input
                                        type="checkbox"
                                        value="{{ $item->id }}"
                                        wire:model.live="selectedItemIds"
                                        class="w-4 h-4 rounded border-purple-200 text-pink-500 focus:ring-pink-400"
                                    />
                                </label>
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <span
                                        class="text-xs px-2 py-0.5 rounded-full font-medium inline-block mt-1"
                                        style="{{ $item->product->category === 'manga' ? 'background:#EEF2FF; color:#4338CA;' : 'background:#FDF2F8; color:#9D174D;' }}"
                                    >
                                    ...
                                    <div class="flex items-center gap-3 mt-3">
                                        <button
                                            ...
                                            class="w-8 h-8 rounded-full border-2 border-purple-200 font-bold text-purple-700 hover:bg-purple-100 transition flex items-center justify-center"
                                        >
                                            −
                                        </button>
                                        <span class="font-semibold w-6 text-center text-gray-800">{{ $item->quantity }}</span>
                                        <button
                    ...
                    <a href="{{ route('products') }}" style="color: #5A2A6E;" class="inline-flex items-center gap-2 text-sm font-medium mt-4 hover:underline">
                        ← Continue Shopping
                    </a>
                </div>
                    ...
                    <div class="bg-white rounded-2xl border border-purple-100 p-5 sticky top-20">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between text-gray-500">
                                <span>Subtotal ({{ $selectedItemCount }} {{ Str::plural('item', $selectedItemCount) }})</span>
                            ...
                            @if($selectedSubtotal > 0 && $selectedSubtotal < 500)
                                <p class="text-xs text-gray-400">
                                    Add ₱{{ number_format(500 - $selectedSubtotal, 2) }} more for free shipping!
                                </p>
                            @endif
                        </div>

                        <div class="border-t border-purple-100 mt-4 pt-4 flex justify-between font-bold text-lg" style="color: #5A2A6E;">
                            <span>Total</span>
                        ...
                        <button
                            ...
                            style="background: #E94E77;"
                            class="w-full text-center text-white font-semibold py-3 rounded-xl mt-5 hover:opacity-90 transition text-sm disabled:opacity-60"
